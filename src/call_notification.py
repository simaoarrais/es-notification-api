...
def send_notification(body):
    """
    Given a Json body, sends a Call notification
    ...
    ----------
        sid : Str
            Unique call ID composed by 34 char string starting with CA
    ...
    try:
        message_properties = client.calls.create(
            from_ = '+18502047786',
            to = body_json["phone_number"],
            twiml = f'<Response><Say>{body_json["message"]}</Say></Response>'
        )
